...
- As of 2026-05-28, registration temporarily does not require an email verification code because the third-party email/verification service is not connected yet. Verification codes are expected to return after that provider is connected. Until then, the public registration form should not show username or verification-code fields, and the backend should auto-generate a unique internal `username` from the email to satisfy existing routes/schema.
