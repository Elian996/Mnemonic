...
import {
  approveAiExtensionDraftInlineAction,
  rejectAiExtensionDraftAction,
  updateAiExtensionDraftInlineAction,
  undoAiExtensionDraftApprovalInlineAction
} from "@/lib/services/ai-extension-service";
...
export type AiExtensionReviewGridItem = {
  id: string;
  word: string;
  phonetic: string;
  partOfSpeech: string;
...
export function AiExtensionReviewGrid({
  ...
  const approveActive = useCallback(async () => {
    if (!activeItem || isApproving || activeItem.targetHasActiveCard) return;
    ...
    setMessage("");
    try {
      await approveAiExtensionDraftInlineAction(activeItem.id);
  ...
  const undoLastApproval = useCallback(async () => {
    ...
    setMessage("");
    try {
      await undoAiExtensionDraftApprovalInlineAction(last.item.id);
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (isAiExtensionDraftEditorOpen()) return;
...
function AiExtensionReviewCardDialog({
  item,
  onClose,
  onPrevious,
  onNext,
  ...
  onApprove: () => void;
  onUndo?: () => void;
  onItemUpdate: (item: AiExtensionReviewGridItem) => void;
  ...
  const saveDraft = useCallback(
    async (closeAfterSave: boolean) => {
      ...
      setEditorMessage("");
      try {
        const result = await updateAiExtensionDraftInlineAction({
          draftId: item.id,
        ...
        lastSavedSignatureRef.current = draftEditSignature(
          editableDraftContent(updatedItem),
          updatedRelatedWords
        );
  ...
  return (
    <div className="pointer-events-auto fixed inset-0 z-[70]" role="presentation">
      ...
      <article
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`${item.word} AI 延伸单词卡`}
        data-ai-extension-draft-editor={isEditing ? "true" : undefined}
              ...
              {isEditing ? (
                ...
                <>
                  ...
                  <button
                    type="button"
                    onClick={onPrevious}
                    aria-label="上一张 AI 延伸单词卡，快捷键左方向键"
                    title="上一张 (←)"
                    className="flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-md border border-[#d8dde6] text-[#69717f] transition hover:border-[#171a1f] hover:text-[#171a1f] dark:border-border dark:text-muted-foreground dark:hover:border-foreground dark:hover:text-foreground"
                  >
                  ...
                  <button
                    type="button"
                    onClick={onNext}
                    aria-label="下一张 AI 延伸单词卡，快捷键右方向键"
                    title="下一张 (→)"
                    className="flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-md border border-[#d8dde6] text-[#69717f] transition hover:border-[#171a1f] hover:text-[#171a1f] dark:border-border dark:text-muted-foreground dark:hover:border-foreground dark:hover:text-foreground"
                  >
                    <span aria-hidden="true">›</span>
                  ...
                  <Badge className={cn("rounded-full px-3 py-1 text-xs", item.targetHasActiveCard ? "bg-amber-100 text-amber-900" : "bg-emerald-100 text-emerald-900")}>
                    {item.targetHasActiveCard ? "目标已有卡" : "目标缺卡"}
                  </Badge>
                </>
              )}
              <button
                type="button"
                onClick={onClose}
                aria-label="关闭 AI 延伸单词卡，快捷键 Esc"
                title="关闭 (Esc)"
                className="flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-md border border-[#d8dde6] text-[#69717f] transition hover:border-[#171a1f] hover:text-[#171a1f] dark:border-border dark:text-muted-foreground dark:hover:border-foreground dark:hover:text-foreground"
              >
                <X className="h-4 w-4" />
          ...
          <section className="mt-4 border-t border-[#eef2f6] pt-4 dark:border-border">
            ...
            {isEditing ? (
              <MemoryCardEditFields
                ...
                statusLabel={aiDraftSaveStatusLabel(autoSaveStatus)}
        ...
        <footer className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-t border-[#f0f0f2] bg-white px-5 py-4 dark:border-border dark:bg-[#1c1c1e]">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-sm font-semibold text-[#111827] dark:text-foreground">{item.payload.baseWord}</span>
              <ArrowRight className="h-4 w-4 text-[#94a3b8]" />
              <span className="text-sm font-semibold text-[#0f62b5]">{item.payload.targetWord}</span>
            ...
            <p className="mt-1 line-clamp-2 text-sm leading-6 text-[#64748b] dark:text-muted-foreground">{item.payload.explanation}</p>
          </div>
          <div className="flex shrink-0 flex-wrap gap-2">
            {onUndo ? (
              <Button
                ...
                size="sm"
                variant="outline"
                disabled={isUndoing || isApproving || isEditing || isSavingDraft}
                onClick={onUndo}
                title="撤回上一次批准 (⌘Z / Ctrl+Z / Shift+R)"
            ...
            <Button
              type="button"
              size="sm"
              disabled={item.targetHasActiveCard || isApproving || isUndoing || isEditing || isSavingDraft}
              onClick={onApprove}
              className="h-9 rounded-full"
            >
              <Check className="mr-1.5 h-4 w-4" />
              {isApproving ? "批准中" : isEditing || isSavingDraft ? "先保存草稿" : "批准成卡"}
            </Button>
            <form action={rejectAiExtensionDraftAction}>
              <input type="hidden" name="draftId" value={item.id} />
              <Button
                type="submit"
                size="sm"
                variant="outline"
                disabled={isApproving || isUndoing || isEditing || isSavingDraft}
                className="h-9 rounded-full border-[#d7dde8]"
              >
...
function isAiExtensionDraftEditorOpen() {
  return Boolean(document.querySelector("[data-ai-extension-draft-editor='true']"));
}

function isAiExtensionUndoShortcut(event: KeyboardEvent) {
...
function editableDraftContent(item: AiExtensionReviewGridItem) {
...
function aiDraftSaveStatusLabel(status: "idle" | "pending" | "saving" | "saved" | "error") {
  if (status === "pending") return "检测到修改，3 秒后自动保存";
  if (status === "saving") return "正在保存...";
  if (status === "saved") return "已保存";
  if (status === "error") return "保存失败";
  return "可直接改写草稿，保存后再批准成卡";
}
