...
- As of 2026-05-31, all newly added loading states should use the shared loading box/line system from `src/components/loading-line.tsx` (`LoadingBox`, `LoadingOverlay`, `LoadingLine`, or `InlineLoadingLine`) instead of ad hoc `Loader2` spinners or plain text. Full-page route loading should use `LoadingBox`; blocking in-place mutations should use `LoadingOverlay` or a compact `LoadingBox`; inline search/opening indicators should use `InlineLoadingLine`.
- As of 2026-05-31, full-page loading screens must not show visible Chinese text on either mobile or desktop. Keep the loading surface graphic-only, using the shared loading line/box animation. Do not render labels such as `正在加载页面`, `正在打开词库`, or `正在打开管理员中心` visibly inside loading boxes.
...
- As of 2026-05-31, word-card APIs may reuse stored `contentHtml` for speed, but they must still normalize upload images in stored HTML before sending it to desktop or mobile clients. Older database HTML can contain original `/uploads/...png` or `.jpg` sources even when `.display.webp` variants exist; the API response should serve the display WebP path, keep `data-original-src`, and set lazy/async image attributes. Do not bypass this display-image rewrite as a performance shortcut.
- As of 2026-05-31, legacy ECDICT phonetic imports may contain visible encoding artifacts such as `(?@)`, bare `?`, escaped backslashes, `^`, `ә`, ASCII `:`, apostrophe stress, or comma stress. These must be normalized before display/import through `src/lib/phonetics.ts`; do not let raw artifacts into `Word.phoneticUk` or `Word.phoneticUs`. The 2026-05-31 repair used `WORD_PHONETIC_ARTIFACT_REPAIR` audit logs and cleaned 830 production words, including `psychiatric`.
...
- As of 2026-05-31, entering account surfaces from a word/level/public page must preserve the source page on both desktop and mobile. The public top-bar account icon should send logged-in users to `/me?from=<current-path>` and logged-out users to `/login?next=<current-path>`; `/me` should show `返回上级` instead of `首页` when `from` is safe; login/register pages should replace the left `首页` link with `返回上级` when `next` or same-origin referer points back to a content page, and successful login/register should return to that source path. Do not regress to a hard-coded homepage breadcrumb for this flow.
...
- As of 2026-05-31, `/repository` has an admin-only `scope=wordPack002` labeled `单词包002`. It is the scanned/OCR-polluted-then-repaired card pack discussed from the `blunt` investigation: build it server-side from `AuditLog.action = MNEMONIC_DUPLICATE_TAKE_YOU_BACK_BATCH_CLEANUP` on `entityType = MnemonicEntry`, plus the same-history extras `salmon` and `sodium`, which were OCR-contaminated but repaired through different actions. The pack should deduplicate to 43 words when no exclusions are recorded. It must use the ordinary repository word-list/card surface and word-card popup/tray, not a custom dense table. Removing a word from this pack records `AuditLog.action = REPOSITORY_WORD_PACK_002_EXCLUDE` on `entityType = Word` and must not delete or archive the word or its mnemonic cards.
- Repository virtual-pack removal must feel instant and quiet. For `wordPack002` and similar audit-log-backed packs, clicking the card `X` or bulk "移出已选" should optimistically remove the item(s) from the visible list immediately, show only a compact top runtime notice, and restore the visible item(s) if the API fails. Do not place `LoadingBox`/`LoadingOverlay` inside a word card for this flow, and do not trigger `revalidatePath("/repository")` from the no-store pack-exclusion API just to save an audit log.
...
- As of 2026-05-31, administrator-center approval undo must not depend only on browser memory. For AI extension review and standalone AI-generated word-card review, the page must be able to recover the latest safe undo target from server state (`ImportDraft.status = SAVED` plus `savedEntryId` pointing at a non-archived generated official card with the review marker). The undo entry must remain visible after refresh, navigation, or when the pending list is empty, and `⌘Z`/`Ctrl+Z`/`Shift+R` should use that recovered target when no in-session approval history exists.
...
Local phone-access and stale Next cache lesson from 2026-05-31: when the user opens the local site from a phone, use the Network URL printed by Next, currently `http://10.234.235.20:3001/` on this Wi-Fi, not `localhost`, and make sure the port `:3001` is present even if the mobile browser address bar visually truncates it. After opening, verify both the root page and a database-backed level route from the Mac against the LAN host, for example:

```bash
curl -sS -o /dev/null -w 'home:%{http_code}\n' http://10.234.235.20:3001/
curl -sS -L -o /tmp/mnemonic-level-check.html -w 'level:%{http_code} %{url_effective}\n' http://10.234.235.20:3001/levels/cet6
rg -n "Runtime Error|Cannot find module|vendor-chunks/zod" /tmp/mnemonic-level-check.html || true
```

If the phone shows the Next.js red Runtime Error overlay with `Cannot find module './vendor-chunks/zod.js'` or another missing `.next/server/vendor-chunks/*.js` file, treat it as a stale/incomplete local `.next` development cache, not as a vocabulary-page product bug. Confirm with `test -f .next/server/vendor-chunks/zod.js` and by curling the affected route. Fix by stopping the listener on port `3001`, preserving the broken cache for inspection, and restarting through the wrapper so Prisma and routes are rebuilt together:

```bash
lsof -nP -iTCP:3001 -sTCP:LISTEN
kill <dev-server-pid>
mv .next ".next.broken-$(date +%Y%m%d%H%M%S)"
npm run dev
```

Then re-check that `.next/server/vendor-chunks/zod.js` exists, `http://localhost:3001/levels/cet6` and `http://<Mac-Wi-Fi-IP>:3001/levels/cet6` return `200`, and the phone no longer shows `Runtime Error`. Do not stop at a homepage `200` after this error, because the missing vendor chunk may only appear when a route imports `zod` or another server dependency.

Also from 2026-05-31: do not assume every desired mobile direction in this document is already implemented in the working tree. The AI document says mobile `/` and `/words` should eventually open into a `选择词库` 3D ring-style navigation using `public/assets/mobile-ring-scene.png`, but the current checked-out local code did not contain that asset or implementation and still rendered the Apple-style mobile homepage. When reporting mobile behavior, verify the actual source/assets and browser state first; if the document describes an intended direction that is missing in code, call it out as backlog or implement it instead of claiming it is live.
...
- 2026-05-31 production-to-local refresh for debugging admin undo: production B was treated as latest data. Server dump `/home/ubuntu/Mnemonic/backups/db-dumps/server-source-for-local-20260531-140333.dump` was copied to `backups/db-dumps/server-source-for-local-20260531-140333.dump`; local pre-refresh backup is `backups/db-dumps/local-before-production-refresh-20260531-140453.dump`. Local PostgreSQL was restored from the server dump, and uploads were refreshed additively with `rsync --ignore-existing`, transferring `8` files. Verification after refresh matched server counts: `Word=41794`, `MnemonicEntry=13928`, `ImportDraft=14832`, `MemoryLink=13351`, `AuditLog=32268`, `public/uploads=1562`; orphan checks returned `orphanMemoryLinksBySource=0`, `orphanMemoryLinksByTarget=0`, and `orphanVersions=0`.
...
- Current public production version is `2.7`, set by the completed 2026-06-03 upload hotlink/origin detection code deploy.
...
- Version `1.5`, 2026-05-30: code-only deploy for the memory-card/loading experience. Code commit `0f41f34` added the shared `LoadingLine`, global route loading surface, line-style search loading indicators, level loading reuse, and memory-card white-card mode; it also kept the production sync/data-merge rules updated in this document. CLI GitHub push failed again with `could not read Username for 'https://github.com': Device not configured`, so the commit was transferred to Tencent Cloud by `git bundle` (`tmp/deploy-bundles/mnemonic-code-0f41f34.bundle`) and `/home/ubuntu/Mnemonic` fast-forwarded from `ac92da9` to `0f41f34`. Local preflight passed `git diff --check`, `npm run typecheck -- --pretty false`, and `npm run build` with existing ESLint warnings only. Server passed `npm ci --no-audit --no-fund`, `npm run db:deploy` with no pending migrations, and `NODE_OPTIONS=--max_old_space_size=1536 npm run build`; `mnemonic.service` restarted and stayed `active`. Data restore and upload sync were not performed because this was code-only. Core production counts remained `Word=41794`, `MnemonicEntry=13893`, and `ImportDraft=14832`. Health checks: `http://mnemonic.top/`, `http://www.mnemonic.top/`, `http://124.221.123.13:3000/levels/cet6`, and `/api/word-card/memory` returned `200 OK` / real JSON.
- Version `1.6`, 2026-05-30: code-only deploy for mobile memory-card interaction fixes. Code commit `641bba6` enlarged mobile touch targets, replaced the three-button theme selector with a single three-state cycle button, expanded the mobile usage manual, fixed mobile memory-card viewport positioning across scrolled lists and touch-device landscape, avoided mobile body fixed-position scroll-lock drift, and restored list positioning to the active word after closing a rapidly navigated memory card. CLI GitHub push failed with `could not read Username for 'https://github.com': Device not configured`, and `gh auth status` showed no logged-in GitHub host, so GitHub remains unsynced until GitHub Desktop or CLI auth is used. The commit was transferred to Tencent Cloud by `git bundle` (`tmp/deploy-bundles/mnemonic-code-641bba6.bundle`) and `/home/ubuntu/Mnemonic` fast-forwarded from `5d195a6` to `641bba6`. Local preflight passed `git diff --check`, `npm run typecheck`, and `npm run build` with existing ESLint warnings only. Server passed `npm ci --no-audit --no-fund`, `npm run db:deploy` with no pending migrations, and `NODE_OPTIONS=--max_old_space_size=1536 npm run build`; `mnemonic.service` restarted and stayed `active`. Data restore and upload sync were not performed because this was code-only. Current production counts after deploy: `Word=41794`, `MnemonicEntry=13905`, `ImportDraft=14832`, and `public/uploads=1554` files. Health checks: `http://124.221.123.13:3000/`, `http://mnemonic.top/`, `http://124.221.123.13:3000/levels/cet6`, and `/api/word-card/memory` returned `200 OK` / real JSON.
- Version `1.7`, 2026-05-31: code-only deploy for the account return-flow fix and administrator `单词包002`. Code commit `3ac3ae2` added safe source-page return helpers, account-topbar/login/register `/me` return behavior, and `/repository?scope=wordPack002`, which is built server-side from `MNEMONIC_DUPLICATE_TAKE_YOU_BACK_BATCH_CLEANUP` plus `salmon` and `sodium`. CLI GitHub push still failed with `could not read Username for 'https://github.com': Device not configured`, so the commit was transferred to Tencent Cloud by `git bundle` (`tmp/deploy-bundles/mnemonic-code-3ac3ae2.bundle`) and `/home/ubuntu/Mnemonic` fast-forwarded from `1373406` to `3ac3ae2`. Local preflight passed `git diff --check`, `npm run typecheck -- --pretty false`, `npm run build`, and a local server-render check of `/repository?scope=wordPack002&reveal=1` showing 43 words. Server passed `npm ci --no-audit --no-fund`, `npm run db:deploy` with no pending migrations, and `NODE_OPTIONS=--max_old_space_size=1536 npm run build`; `mnemonic.service` restarted and stayed `active`. Data restore and upload sync were not performed because this was code-only. Current production counts after deploy: `Word=41794`, `MnemonicEntry=13906`, `ImportDraft=14832`, `wordPack002=43`, and `public/uploads=1554` files. Health checks: server-rendered `http://127.0.0.1:3000/repository?scope=wordPack002&reveal=1` with an admin session returned `200` and contained `单词包002`, `acidity`, `blunt`, `salmon`, and `sodium`; `http://127.0.0.1:3000/` and `/api/word-card/memory` returned `200`; `http://124.221.123.13:3000/levels/cet6` returned `200`. Public `mnemonic.top` and `www.mnemonic.top` currently redirect to Tencent DNSPod webblock URLs, so use direct server/IP health checks until the domain/DNS block is handled.
- Version `1.8`, 2026-05-31: code-only deploy for faster repository word-pack removal and unified loading-box UI. Code commit `0a91006` added `/api/repository/word-pack-exclusions`, moved shared pack-exclusion writes into `src/lib/repository-pack-exclusions.ts`, made single-card and bulk "移出词包" use fetch plus optimistic removal instead of server-action redirect/full-page reload, added runtime pack-removal notices, introduced `LoadingBox`/`LoadingOverlay`/`InlineLoadingLine`, and switched route/repository loading states to the shared loading-box system. CLI GitHub push was not retried because credentials are still unavailable; the commit was transferred to Tencent Cloud by `git bundle` (`tmp/deploy-bundles/mnemonic-code-0a91006.bundle`) and `/home/ubuntu/Mnemonic` fast-forwarded from `af75414` to `0a91006`. Local preflight passed `git diff --check`, `npm run typecheck -- --pretty false`, `npm run build`, local `http://localhost:3001/` and `/levels/cet6` checks, and a local pack-removal API test that returned `200` in about `0.295s` and was cleaned up. Server passed `npm ci --no-audit --no-fund`, `npm run db:deploy` with no pending migrations, and `NODE_OPTIONS=--max_old_space_size=1536 npm run build`; `mnemonic.service` restarted and stayed `active`. Data restore and upload sync were not performed because this was code-only. A controlled production API test temporarily removed `cape` from `wordPack002`, returned `200` in `169ms`, then deleted the test audit log; the production visible pack count returned to `36` after existing user/admin exclusions. Current production counts after deploy: `Word=41794`, `MnemonicEntry=13912`, `ImportDraft=14832`, and `public/uploads=1554` files. Health checks: `http://124.221.123.13:3000/levels/cet6` and `/api/word-card/memory` returned `200`.
- Version `1.9`, 2026-05-31: code-only deploy for repository word-pack removal polish after the user reported that the in-card loading box was still slow and ugly. Code commit `e69aec2` removed the card-level `LoadingBox`/`LoadingOverlay` for pack removal, made single-card removal hide the card immediately through `.mn-repository-word-card-hidden { display: none !important; }`, made bulk pack removal optimistically remove selected rows before the API returns, added `pending/done/error` runtime notice states, and removed `revalidatePath("/repository")` from `/api/repository/word-pack-exclusions`. Lesson: do not rely on the HTML `hidden` attribute to hide Tailwind `flex` cards because the author `display:flex` rule can keep them visible; use a dedicated class or state-rendered removal. Local verification passed `git diff --check`, `npm run typecheck -- --pretty false`, `npm run build`, and a Chrome/Playwright visual test on `http://localhost:3001/repository?sort=az&view=grid&reveal=1&scope=wordPack002`: clicking `acidity` hid the card in about `45ms`, showed only the compact top notice `已从词包移出「acidity」。单词和单词卡仍然保留。`, and the temporary audit log was cleaned up (`acidity` exclusion count returned to `0`). CLI GitHub push was not retried because credentials are still unavailable; the commit was transferred to Tencent Cloud by `git bundle` (`tmp/deploy-bundles/mnemonic-code-e69aec2.bundle`) and `/home/ubuntu/Mnemonic` fast-forwarded from `45de054` to `e69aec2`. Server passed `npm ci --no-audit --no-fund`, `npm run db:deploy` with no pending migrations, and `NODE_OPTIONS=--max_old_space_size=1536 npm run build`; `mnemonic.service` restarted and stayed `active`. A controlled production API test temporarily removed `chronicle` from `wordPack002`, returned `200` in `156ms`, then deleted the test audit log (`chronicle` exclusion count returned to `0`); production visible `wordPack002` count before the test was `35`. Data restore and upload sync were not performed because this was code-only. Current production counts after deploy: `Word=41794`, `MnemonicEntry=13913`, `ImportDraft=14832`. Health checks: `http://127.0.0.1:3000/`, `/levels/cet6`, and `/api/word-card/memory` returned `200`.
- Version `2.0`, 2026-05-31: deploy for administrator review undo persistence plus production-to-local data refresh. Code commit `00946c2` added server-derived latest undo targets for `AI延伸审核` and `AI生成单词卡`, exposed a page-level undo bar even when the pending list is empty, and made dialog/keyboard undo use recovered server state after refresh or navigation. Before coding, local A was refreshed from production B using `/home/ubuntu/Mnemonic/backups/db-dumps/server-source-for-local-20260531-140333.dump`; local pre-refresh backup is `backups/db-dumps/local-before-production-refresh-20260531-140453.dump`; uploads were refreshed additively with `rsync --ignore-existing`, transferring `8` files. Local verification passed `git diff --check`, `npm run typecheck -- --pretty false`, `npm run build`, direct helper checks showing latest undo targets `yellowy` and `antenna`, and Chrome/Playwright desktop/mobile visual checks showing the recovered undo bars. CLI GitHub push was not retried because credentials are still unavailable; the commit was transferred to Tencent Cloud by `git bundle` (`tmp/deploy-bundles/mnemonic-code-00946c2.bundle`) and `/home/ubuntu/Mnemonic` fast-forwarded from `d86915f` to `00946c2`. Server passed `npm ci --no-audit --no-fund`, `npm run db:deploy` with no pending migrations, and `NODE_OPTIONS=--max_old_space_size=1536 npm run build`; `mnemonic.service` restarted and stayed `active`. Production server-render checks with an admin session showed `最近通过：antenna / 撤回 antenna` on `scope=aiGeneratedWordCards` and `最近批准：yellowy / 撤回 yellowy` on `scope=aiExtensionReview`. Current production counts after deploy: `Word=41794`, `MnemonicEntry=13928`, `ImportDraft=14832`, `MemoryLink=13351`, `AuditLog=32268`, and `public/uploads=1562` files. Health checks: `http://127.0.0.1:3000/`, `/levels/cet6`, and `/api/word-card/memory` returned `200` / real JSON.
- Version `2.1`, 2026-05-31: code-only deploy for graphic-only loading screens after the user reported mobile/desktop loading pages should not show Chinese text. Code commit `e79f5f7` made `LoadingBox` render only the shared loading line, changed default loading labels to `Loading`, and updated app/repository/level route loading surfaces so `正在加载页面`, `正在打开管理员中心`, and `正在打开词库` are not visible. CLI GitHub push was not retried because credentials are still unavailable; the commit was transferred to Tencent Cloud by `git bundle` (`tmp/deploy-bundles/mnemonic-code-e79f5f7.bundle`) and `/home/ubuntu/Mnemonic` fast-forwarded from `11a347a` to `e79f5f7`. Local preflight passed `git diff --check`, `npm run typecheck -- --pretty false`, `npm run build`, and a source check that no route loading file still passed the removed Chinese labels. Server passed `npm ci --no-audit --no-fund`, `npm run db:deploy` with no pending migrations, and `NODE_OPTIONS=--max_old_space_size=1536 npm run build`; `mnemonic.service` restarted and stayed `active`. Data restore and upload sync were not performed because this was code-only. Current production counts were not changed from v2.0 (`Word=41794`, `MnemonicEntry=13928`, `ImportDraft=14832`, `public/uploads=1562`). Health checks: `http://127.0.0.1:3000/`, `/levels/cet6`, and `/api/word-card/memory` returned `200` / real JSON.
- Version `2.2`, 2026-05-31: code-only deploy for broad website speed optimization after the user asked to find and fix slow open/close/loading paths. Code commits `2d704d6` and `7ec0037` made `/api/word-card/[slug]` reuse stored `contentHtml` instead of rerendering markdown on every GET, added a 60-second server-side seed cache for level/random word pools, shared the word-card popup client cache across buttons on the same page, stopped loading AI review draft lists unless their repository scope is active, parallelized administrator center data loading, and added a 30-second cache for non-active admin overview/stat counters while keeping the current active scope fresh. CLI GitHub push was not retried because credentials are still unavailable; the commits were transferred to Tencent Cloud by `git bundle` (`tmp/deploy-bundles/mnemonic-code-2d704d6.bundle` and `tmp/deploy-bundles/mnemonic-code-7ec0037.bundle`) and `/home/ubuntu/Mnemonic` fast-forwarded from `316be5c` to `7ec0037`. Local preflight passed `git diff --check`, `npm run typecheck -- --pretty false`, `npm run build`, and production-mode local timing on `http://127.0.0.1:3002`: repeated `/levels/random?sort=random&seed=perf` was about `0.056s`, `/api/word-card/memory` about `0.006s`, `wordPack002` repository about `0.029s`, and `aiGeneratedWordCards` repository about `0.028s`. Server passed `npm ci --no-audit --no-fund`, `npm run db:deploy` with no pending migrations, and `NODE_OPTIONS=--max_old_space_size=1536 npm run build`; `mnemonic.service` restarted and stayed `active`. Data restore and upload sync were not performed because this was code-only. Server timing after restart/warmup: `/levels/random?sort=random&seed=perf` about `0.208s`, `/api/word-card/memory` about `0.009s`, `/api/word-search?q=mem&limit=20` about `0.054s`, `/repository?scope=wordPack002` with admin session about `0.033s`, `/repository?scope=missingCards` about `0.220s`, and `/repository?scope=aiGeneratedWordCards` about `0.048s`; all returned `200`. Current production counts remained `Word=41794`, `MnemonicEntry=13928`, `ImportDraft=14832`, and `public/uploads=1562`.
- Version `2.3`, 2026-05-31: code-only deploy for the mobile bug where tapping the memory-card close `X` from an anonymous level/word-learning page could close the tray and then fall through to the underlying login/account button, navigating to `/login`. Code commit `5e25567` changed the mobile memory-card close button so pointer events only stop propagation and the actual close happens on the button click, with `preventDefault`/`stopPropagation`, preventing the component from unmounting before the browser dispatches its compatibility click. CLI GitHub push was not retried because credentials are still unavailable; the commit was transferred to Tencent Cloud by `git bundle` (`tmp/deploy-bundles/mnemonic-code-5e25567.bundle`) and `/home/ubuntu/Mnemonic` fast-forwarded from `82dd2d6` to `5e25567`. Local preflight passed `git diff --check`, `npm run typecheck -- --pretty false`, `npm run build`, and a Chrome mobile/touch Playwright check on `http://127.0.0.1:3002/levels/cet6?sort=az`: open first visible mobile word row, tap `关闭助记卡`, URL stayed on `/levels/cet6?sort=az`, and the tray count became `0`. Server passed `npm ci --no-audit --no-fund`, `npm run db:deploy` with no pending migrations, and `NODE_OPTIONS=--max_old_space_size=1536 npm run build`; `mnemonic.service` restarted and stayed `active`. Production Chrome mobile/touch verification against `http://124.221.123.13:3000/levels/cet6?sort=az` passed the same close-flow check. Data restore and upload sync were not performed because this was code-only. Current production counts remained `Word=41794`, `MnemonicEntry=13928`, `ImportDraft=14832`.
- Version `2.4`, 2026-05-31: code-only deploy for the server/desktop word-card image regression where opening cards such as `poke` still loaded the original multi-megabyte PNG even though display WebP variants already existed. Root cause: the Version `2.2` speed optimization reused stored `MnemonicEntry.contentHtml` to avoid repeated Markdown rendering, but older stored HTML still contained original `/uploads/...png` paths. Code commit `46282f1` added `prepareMnemonicHtmlForDisplay`, made `/api/word-card/[slug]` apply the display-image rewrite only when stored HTML contains images, and added a unit test for rewriting old HTML to `.display.webp` while preserving `data-original-src`. CLI GitHub push was not retried because credentials are still unavailable; the commit was transferred to Tencent Cloud by `git bundle` (`tmp/deploy-bundles/mnemonic-code-46282f1.bundle`) and `/home/ubuntu/Mnemonic` fast-forwarded from `b3edcf2` to `46282f1`. Local verification passed `npm test -- tests/unit/wiki-renderer.test.ts`, `npm run typecheck -- --pretty false`, `npm run build`, `git diff --check`, and a direct `poke` renderer check showing the image rewritten to `.display.webp`. Server passed `npm ci --no-audit --no-fund`, `npm run db:deploy` with no pending migrations, `npm run images:optimize` (`781` fresh display variants, `0` created, `0` failed), and `NODE_OPTIONS=--max_old_space_size=1536 npm run build`; `mnemonic.service` restarted and stayed `active`. Data restore and upload sync were not performed because this was code-only and the display variants already existed. Production verification: `http://124.221.123.13:3000/api/word-card/poke` returned the image as `/uploads/generated-mnemonic-images/poke-cmov1im6-1779620994989.display.webp` with `data-original-src` pointing to the original PNG; the WebP downloaded `24,304` bytes in about `0.086s`, while the original PNG is `1,957,123` bytes and took about `3.605s`. Current production counts remained `Word=41794`, `MnemonicEntry=13928`, `ImportDraft=14832`, `MemoryLink=13351`, and `public/uploads=1562` files.
- Version `2.5`, 2026-05-31: production data-only repair for visible phonetic artifacts after `psychiatric` displayed `/saiki'ætrik; (?@) si-/`. No code deployment or GitHub push was performed for production; the server code commit remained `571097c`. Before the production mutation, the server database was backed up to `/home/ubuntu/Mnemonic/backups/db-dumps/server-before-phonetic-artifact-repair-20260531-183719.dump`. A narrow Prisma data patch normalized `Word.phoneticUk` and `Word.phoneticUs` for legacy ECDICT artifacts (`(?@)`, `?`, escaped backslashes, `^`, `ә`, ASCII length/stress markers), updated `830` words, and wrote `830` `WORD_PHONETIC_ARTIFACT_REPAIR` audit logs. Local source dump and upload sync were not performed because this was not a restore and uploads were unchanged. Production report path: `/home/ubuntu/Mnemonic/tmp/phonetic-artifact-repair/phonetic-artifact-repair-2026-05-31T10-38-02-540Z.json`; local report path: `/Users/mr.mao/Desktop/Mnemonic/tmp/phonetic-artifact-repair/phonetic-artifact-repair-2026-05-31T10-36-18-795Z.json`. Final production counts: `Word=41794`, `MnemonicEntry=13932`, `ImportDraft=14832`, `AuditLog=33110`, and `public/uploads=1566` files. Verification: production dirty phonetic artifact count returned `0`; `psychiatric` now has `/saikiˈætrik/`; `http://124.221.123.13:3000/api/word-card/psychiatric` returned `/saikiˈætrik/`; homepage returned `200`; `mnemonic.service` was `active`.
- Version `2.6`, 2026-05-31: code deploy plus production-to-local data refresh after the user asked to read the AI document and synchronize code/data. Behavioral code commit `c2c0a9a` (`Harden APIs and normalize phonetics`) was pushed to GitHub `origin/main` via GitHub Desktop after CLI `git push origin main` failed with `could not read Username for 'https://github.com': Device not configured`; local `HEAD` and `origin/main` both verified as `c2c0a9a`. The commit added shared phonetic normalization/prevention code, a repeatable phonetic artifact repair script/test, and API hardening around request bodies, client IP handling, rate limits, upload hotlink checks, registration/verification, word-card/repository routes, and contact email sending. Local verification before deploy passed `npm run typecheck -- --pretty false`, `npm run build`, `npm test`, and `git diff --check`. Tencent Cloud `/home/ubuntu/Mnemonic` fast-forwarded from `571097c` to `c2c0a9a`; server passed `npm ci --no-audit --no-fund`, `npm run db:deploy` with no pending migrations, and `NODE_OPTIONS=--max_old_space_size=1536 npm run build`; `mnemonic.service` restarted and stayed `active`. Production data was not overwritten from local. Instead, production B was treated as source of truth and refreshed back into local A: server dump `/home/ubuntu/Mnemonic/backups/db-dumps/server-source-for-local-20260531-194111.dump` was copied to `backups/db-dumps/server-source-for-local-20260531-194111.dump`; local pre-refresh backup is `backups/db-dumps/local-before-production-refresh-20260531-194229.dump`; local PostgreSQL was restored from the server dump; local uploads were refreshed additively with `rsync --ignore-existing`, transferring `4` files. Final local/server counts matched: `Word=41794`, `MnemonicEntry=13932`, `ImportDraft=14832`, `MemoryNode=13976`, `MemoryLink=13353`, `AuditLog=33110`, `Bookmark=494`, `WordMark=1170`, and `public/uploads=1566`. Verification: local/server dirty phonetic artifact count returned `0`; orphan checks returned `orphanLinksBySource=0`, `orphanLinksByTarget=0`, and `orphanVersions=0`; public homepage returned `200`; `http://124.221.123.13:3000/api/word-card/psychiatric` returned clean `/saikiˈætrik/`; server repo status had only existing untracked `.env.bak*` files. The later AI_CONTEXT-only commit containing this record does not change production behavior.
- Version `2.7`, 2026-06-03: code-only deploy for the public-server uploaded-image display failure reported on the `peril` word card. Root cause: `/uploads` hotlink protection compared only `request.url` and configured origins, while the production `next start -H 0.0.0.0 -p 3000` request could see an internal origin that did not match the browser's `http://124.221.123.13:3000` Referer; after the first fix, public static-file serving could also bypass the route-level guard on successful requests. Code commits `e0c90d8` and `0b6c4b7` moved upload hotlink origin detection into `src/lib/uploads/hotlink.ts`, allowed same-origin requests through `Host` / `X-Forwarded-Host` / `X-Forwarded-Proto`, kept route-level protection, and added a `/uploads/:path*` middleware guard so static uploaded files are checked before Next's public-file handler. CLI `git push origin main` failed with `could not read Username for 'https://github.com': Device not configured`, so GitHub remains unsynced until GitHub Desktop or CLI auth is used. Tencent Cloud `/home/ubuntu/Mnemonic` was fast-forwarded by git bundles from `486d1c1` to `e0c90d8`, then to `0b6c4b7`; server passed `npm ci --no-audit --no-fund`, `npm run db:deploy` with no pending migrations, and `NODE_OPTIONS=--max_old_space_size=1536 npm run build` for both deploy steps; `mnemonic.service` restarted and stayed `active`. Data restore, local source dump, server pre-restore backup, and upload sync were not performed because this was code-only. Local verification passed `npx tsc --noEmit`, `npm test`, `npm run build`, and `git diff --check HEAD~2..HEAD`. Final production counts after deploy: `Word=41794`, `MnemonicEntry=13935`, `ImportDraft=14832`, and `public/uploads=1568` files. Health checks: `http://124.221.123.13:3000/` returned `200 OK`; `/api/word-card/peril` returned the memory image as `/uploads/editor/1780452938780-cac9cbec-ac8f-4305-98a2-7eeacf279c71-image.display.webp` with `data-original-src`; the same image with `Referer: http://124.221.123.13:3000/word/peril` returned `200 OK`; the same image with `Referer: https://example.net/article` returned `403 Forbidden`.
...
- Version `1.5`, 2026-05-30 improvement: when GitHub CLI credentials are unavailable but the server already has the parent commit, a small `git bundle` is a safe code-only fallback that preserves Git history and avoids rsyncing the working tree. GitHub still needs credentials fixed or a GitHub Desktop push to bring `origin/main` up to the deployed server commit.
- Version `1.7`, 2026-05-31 improvement: for admin-only repository scopes generated from existing `AuditLog` data, no production data patch is needed when the required logs already exist. Deploy only the server-side scope/query code, then verify the pack by rendering `/repository?scope=<scope>` with a short-lived admin session and checking representative words plus the count. Remaining bottlenecks: GitHub CLI credentials are still unavailable, and the public domain currently redirects to DNSPod webblock pages even though direct `:3000` server health checks pass.
- Version `1.8`, 2026-05-31 improvement: expensive admin-list mutations should not use server-action redirect when the user only needs to remove items from a virtual pack. Prefer a narrow no-store API, optimistic client removal, and a temporary runtime notice, then revalidate `/repository` for the next navigation. For production verification of mutating APIs, use a controlled create-and-cleanup audit-log test and report the final restored count.
- Version `2.0`, 2026-05-31 improvement: admin review undo needs a durable server source, not just in-memory UI history. Store enough metadata to reconstruct an undo target from `ImportDraft` and the generated `MnemonicEntry`, show that target on the page outside the modal, and verify the empty-pending-list case because that is when client-only undo most obviously disappears.
- Version `2.1`, 2026-05-31 improvement: for full-page loading states, make the shared component enforce the no-visible-text rule instead of fixing each route one by one. This keeps future desktop/mobile loading pages graphic-only by default.
- Version `2.2`, 2026-05-31 improvement: for speed work, first measure representative public pages, word-card APIs, search, and admin repository scopes before editing. Fast wins in this app are usually avoiding repeated markdown rendering when `contentHtml` already exists, not fetching inactive admin draft lists, parallelizing independent Prisma queries, and using very short server/client caches for read-heavy derived lists or dashboard counters. Keep the active admin scope fresh when mutations can change its visible rows, but it is acceptable for side counters and overview cards to be stale for roughly 30 seconds.
- Version `2.3`, 2026-05-31 improvement: on mobile/touch UIs, do not close/unmount overlays on `pointerup` if a synthetic click can still be dispatched at the same screen coordinate. Let the real button `click` perform the close, or otherwise suppress the follow-up click, so taps on close controls cannot pass through to underlying login/account/navigation buttons. Verify these bugs with a real mobile/touch Playwright context, not only desktop clicks.
- Version `2.4`, 2026-05-31 improvement: when optimizing word-card API speed by reusing stored `contentHtml`, keep a lightweight post-processing step for image delivery. The safe pattern is `reuse stored HTML when present`, `skip work for no-image HTML`, and `rewrite upload images to existing `.display.webp` variants when `<img>` is present`; this avoids the old expensive Markdown render without regressing to multi-megabyte original images on desktop/server card popups. Reusable verification: check a representative old image-backed card, for example `/api/word-card/poke`, and compare the returned `<img>` plus the WebP/PNG byte sizes.
- Version `2.5`, 2026-05-31 improvement: for production data-only cleanup, prefer a narrow audited patch over any database restore. First query exact dirty counts and representative rows, then take a server `pg_dump` backup, run an idempotent data patch that writes per-row audit logs with before/after metadata, and verify both the global dirty count and the exact user-reported word through the public API. Remaining bottleneck: local prevention code can be written and tested independently, but it is not automatically active on production until a later code deploy; mention that distinction when the user asks whether future imports are protected on the live server.
- Version `2.6`, 2026-05-31 improvement: for a combined "代码数据同步" request after a production data patch, the safest split path is `A -> GitHub -> B` for code and `B -> A` for data. This activated the previously local phonetic-prevention code on production while preserving production as the data source of truth; no local database was restored over production. Reusable check: after a B-to-A refresh, compare local and server counts for `Word`, `MnemonicEntry`, `ImportDraft`, `MemoryNode`, `MemoryLink`, `AuditLog`, `Bookmark`, and `WordMark`, plus upload file counts and orphan checks using `"sourceNodeId"`, `"targetNodeId"`, and `"mnemonicEntryId"` column names. Remaining bottlenecks: CLI GitHub credentials are still unavailable, so GitHub Desktop remains the reliable push fallback; local untracked `.claude/`, `design-mockup.html`, and `qr.html` should stay excluded from sync unless the user explicitly asks to keep them.
- Version `2.7`, 2026-06-03 improvement: for `/public`-backed uploaded assets, route handlers are not enough to enforce hotlink policy consistently because Next can serve existing files through its public static-file path. Put the access decision in middleware matched to `/uploads/:path*`, then keep route-level checks as defense in depth. For production image bugs, verify both directions: same-origin Referer returns `200` and unrelated external Referer returns `403`.

### 2026-05-31 v2.6 Sync Retrospective

This sync established the preferred pattern for ambiguous "代码数据同步" requests after public launch:

- Read this AI document first, then classify the request before touching Git, the server, or databases. The winning classification for v2.6 was combined code deploy plus production-to-local refresh, not local-over-production restore.
- Treat code and data as two separate flows: code/migrations/scripts move from local `A` to GitHub and then to Tencent Cloud `B`; production data moves from `B` back to `A` when local needs to match live state.
- Run local verification before deployment: `git diff --check`, `npm run typecheck -- --pretty false`, `npm run build`, and `npm test` when tests exist for the touched behavior.
- Stage intentionally. Do not let GitHub Desktop's "included" untracked files sneak into commits. For v2.6, `.claude/`, `design-mockup.html`, and `qr.html` were intentionally left untracked.
- Prefer CLI `git push origin main`; when it fails with `could not read Username for 'https://github.com': Device not configured`, use GitHub Desktop's `Push origin`, then verify with `git fetch origin` and matching `git rev-parse --short HEAD` / `origin/main`.
- 2026-06-03 GitHub path update: after the user opens GitHub Desktop, Codex can trigger `Repository -> Push` through the macOS menu, then run `git fetch origin` and verify local `HEAD` equals `origin/main`. Once GitHub is synchronized, Codex can autonomously finish normal code deploys end-to-end: server `git fetch/pull`, dependency install, migration deploy, production build, service restart when behavior changed, and public verification. If GitHub Desktop asks for login, 2FA, or reauthorization, the user must handle that prompt; destructive data operations still require explicit confirmation.
- Deploy code on the server by fast-forwarding Git history, then run `npm ci --no-audit --no-fund`, `npm run db:deploy`, build with `NODE_OPTIONS=--max_old_space_size=1536`, restart only when behavior code changed, and verify `mnemonic.service` is `active`.
- For production-to-local refresh, create a fresh server dump, copy it locally, back up local before restore, terminate local DB sessions, restore with `pg_restore --clean --if-exists --no-owner --no-acl`, and refresh uploads additively with `rsync --ignore-existing`.
- Always verify parity after a B-to-A refresh: compare local/server core counts, upload file counts, dirty-data counters relevant to the task, orphan link/version checks, public homepage status, and one representative API such as `/api/word-card/psychiatric`.
- Write the sync record into this document before the final report. If the record itself is committed afterward as docs-only, push it and fast-forward the server repo, but do not restart production for an AI_CONTEXT-only change.

Concrete improvement for future syncs: keep a small reusable verification script or command that prints the standard parity object in one JSON block for both local and server. The v2.6 run exposed that ad hoc raw SQL is easy to get wrong when column names differ from mental memory; use the actual Prisma schema names (`sourceNodeId`, `targetNodeId`, `mnemonicEntryId`) and prefer a single maintained check over hand-written one-off SQL.

Fastest safe order:

1. Scope and optimize the sync path: decide whether the change is docs-only, code-only, schema-migration, uploads-additive, production-to-local refresh, data patch/migration, combined code-plus-B-to-A refresh, or explicit local-over-production restore; review the last sync record; note the fastest safe path and expected bottleneck before running it.
2. Separate code from data unless the user explicitly chooses a data overwrite. Code goes `A -> GitHub -> B`; production data goes `B -> A`; uploads are additive unless an exact mirror is separately confirmed.
3. Local preflight: check `git status --short --branch`, `git fetch origin`, `git diff --check`, `npm run typecheck -- --pretty false`, and `npm run build` when code changed. Run targeted tests or `npm test` when behavior changed. If `prisma/schema.prisma` changed, make sure a matching migration exists before deployment; `npx prisma migrate status` must be clean locally.
4. Commit only intentional files. Exclude untracked local scratch/config files unless the user explicitly asks to keep them. Commit and push code/reports/migrations to GitHub. Prefer CLI `git push origin main`; if HTTPS credentials fail on this Mac, use GitHub Desktop's `Push origin` button and then verify `git rev-parse --short HEAD` equals `git rev-parse --short origin/main`.
   Local-only creative/export folders, especially `小红书宣传/`, are not part of GitHub/server sync unless the user explicitly asks to publish or preserve those assets in Git. Keep generated Xiaohongshu materials local by default.
5. For normal code/schema deployment, do not create or restore a local source dump. On the server, `git pull --ff-only`, install dependencies, run `npm run db:deploy`, build, restart when behavior changed, and verify.
6. Create a server database backup before any migration that is not obviously additive, before any script that mutates production data, before every production-to-local refresh source dump, and before every explicit local-over-production restore. The backup/dump must finish before the risky operation starts.
7. Sync uploads to production only when assets changed and the database already references those paths. Prefer additive upload syncs; use `rsync --delete` from local to production only in an explicit local-over-production restore. For B-to-A refreshes, use additive upload sync from server to local.
8. Verify all affected layers: local/GitHub/server commit hashes match for code deploys; server service is `active`; homepage returns `200 OK`; a representative word-card API returns real JSON; compare local/server table counts after B-to-A refreshes; compare production table counts before/after data patches; compare upload counts and at least one representative uploaded asset when uploads changed; run dirty-data and orphan checks relevant to the task.
9. Before the final report, update this document with the version/sync record and the sync-method iteration note: chosen path, improvement over the previous path, new reusable command/check if any, and unresolved bottleneck if any. For an AI_CONTEXT-only follow-up commit, push and fast-forward the server repo without restarting production.
