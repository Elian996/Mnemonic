    ...
    <InteriorPage>
      ...
      <section className="mx-auto flex min-h-[calc(100vh-88px)] w-full max-w-md items-center px-5 pb-16 sm:px-8">
        <div className="mn-panel w-full p-6">
          ...
          <form action={requestRegisterCodeAction} className="mt-6 grid gap-3">
          ...
          <form action={registerAction} className="mt-5 grid gap-4 border-t border-[var(--mn-line)] pt-5">
...
function registerErrorMessage(error: string | undefined) {
  if (error === "duplicate") return "这个邮箱或用户名已经被注册。";
  ...
  if (error === "invalid") return "请填写有效邮箱、3-32 位用户名和至少 8 位密码。";
  return "";
}
