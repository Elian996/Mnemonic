...
import { requestPasswordResetCodeAction, resetPasswordAction } from "@/lib/auth/actions";
...
export default async function ForgotPasswordPage({
  ...
  const errorMessage = passwordResetErrorMessage(sp.error);
  const successMessage = sp.error === "sent" ? "如果邮箱已注册，验证码会发送到该邮箱。" : "";
    ...
    <InteriorPage>
      ...
      <section className="mx-auto flex min-h-[calc(100vh-88px)] w-full max-w-md items-center px-5 pb-16 sm:px-8">
        <div className="mn-panel w-full p-6">
          ...
          <h1 className="mt-3 font-serif text-5xl font-semibold tracking-normal">重设密码</h1>
          <form action={requestPasswordResetCodeAction} className="mt-6 grid gap-3">
            ...
            <button type="submit" className="h-11 rounded-md border border-[var(--mn-line)] bg-[var(--mn-paper-deep)] px-4 text-sm font-semibold text-[var(--mn-ink)] transition hover:border-[var(--mn-ink)]">
              发送验证码
            </button>
          </form>

          <form action={resetPasswordAction} className="mt-5 grid gap-4 border-t border-[var(--mn-line)] pt-5">
...
function passwordResetErrorMessage(error: string | undefined) {
  if (error === "invalid_email") return "请先填写有效邮箱。";
  if (error === "invalid") return "请填写有效邮箱、6 位验证码和至少 8 位新密码。";
